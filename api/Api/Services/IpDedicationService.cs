...
public sealed class IpDedicationService
{
	...
	private HashSet<int> _usedAddresses;
	...
	public int IpStringToIpInt(string address) // tested
	{
		int slashIndex = address.LastIndexOf('/');
		string actualAddress = slashIndex == -1 ?
			address : address.Substring(0, slashIndex);

		return IpBytesToIpInt(actualAddress.Split('.').Select(byte.Parse).ToArray());
	}
	private int IpBytesToIpInt(byte[] bytes) // tested
	{
	...
	private byte[] IpIntToIpBytes(int index) // tested
	{
	...
	private string IpIntToIpString(int index) // tested
	{
		byte[] address = IpIntToIpBytes(index);
	...
	public string EnsureDedicatedAddressForPeer(string pubKey) // tested
	{
		...
		{
			return IpIntToIpString(address);
		}
		else
		{
			if(_dedicatedAddresses.Count >= MaxClients)
			{
				throw new AggregateException("Max number of clients reached.");
			}
	...
	public void SetState(Dictionary<string, int> keyToIp)
	{
		_dedicatedAddresses = keyToIp;
		_usedAddresses = keyToIp.Values.ToHashSet();
	}
}
